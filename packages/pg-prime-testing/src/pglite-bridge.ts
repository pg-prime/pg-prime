 ...
 *
 * This is the ONE copy (design/13 decision 2). `packages/pg-prime/test/live/_pglite-bridge.ts` is
 * a re-export of this file by relative source path, so the runtime's tier-1 harness keeps its
 * import specifier and there is no workspace cycle — `pg-prime` cannot dev-depend on a package
...
/**
 * The two methods the bridge drives, declared structurally rather than imported from
 * `@electric-sql/pglite`.
 *
 * Same reason `pg-prime`'s driver duck-types `pg` (design/02 §3, `src/driver/pg-like.ts`): PGlite
 * is an OPTIONAL peer of this package, and a `.d.ts` that names it would make the optionality a
 * lie for every consumer whose type-checker reads our declarations. `PGlite` satisfies this —
 * `execProtocolRaw(message, options?)` is assignable to the one-parameter shape below.
 */
...
export function serve(db: PgliteLike, host = '127.0.0.1'): Promise<PgliteBridge> {
  let port = 0
