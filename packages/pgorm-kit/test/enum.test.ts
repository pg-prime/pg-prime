...
const CURRENT_DB = "pgorm_spike_enum_current";
const DESIRED_DB = "pgorm_spike_enum_desired";
const MISORDER_DB = "pgorm_spike_enum_misordered";
const T = 120_000;
...
describe("regression #1: ALTER TYPE … ADD VALUE is ordered before every use of the new label", () => {
  ...
  it(
    ...
    async () => {
      ...
      await expect(writePlan("/tmp/pgorm-should-not-exist", plan)).rejects.toThrow(/refusing to write/);
    },
    T,
  );
});
