 ...
 *  - `PGORM_PG_DUMP_URI`'s `{db}` substitution was not URL-encoded;
...
describe("PGORM_PG_DUMP_URI", () => {
  it("URL-encodes the database it substitutes", async () => {
    const saved = { argv: process.env["PGORM_PG_DUMP"], uri: process.env["PGORM_PG_DUMP_URI"] };
    try {
      process.env["PGORM_PG_DUMP"] = JSON.stringify(["sh", "-c", "echo 'pg_dump (PostgreSQL) 17.11'"]);
      process.env["PGORM_PG_DUMP_URI"] = "postgresql://u@h:5432/{db}";
      ...
      if (saved.argv === undefined) delete process.env["PGORM_PG_DUMP"];
      else process.env["PGORM_PG_DUMP"] = saved.argv;
      if (saved.uri === undefined) delete process.env["PGORM_PG_DUMP_URI"];
      else process.env["PGORM_PG_DUMP_URI"] = saved.uri;
    }
  });
});
    ...
    async () => {
      ...
      const observed = await withClient({ ...ADMIN, database: DB }, async (client) => {
        const report = await applySegments(
          client,
          [stmt(0, "SELECT set_config('pgorm.seen_path', current_setting('search_path'), false)")],
          [{ index: 0, transactional: true, statements: [0] }],
        );
        expect(report.status).toBe("applied");
        const r = await client.query("SELECT current_setting('pgorm.seen_path') AS v");
  ...
  it(
    ...
    async () => {
      const { seen, after } = await withClient({ ...ADMIN, database: DB }, async (client) => {
        const bare = {
          ...stmt(0, "SELECT set_config('pgorm.seen_lock', current_setting('lock_timeout'), false)"),
        ...
        expect(report.status).toBe("applied");
        const a = await client.query("SELECT current_setting('pgorm.seen_lock') AS v");
