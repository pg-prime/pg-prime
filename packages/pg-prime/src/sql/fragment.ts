...
export function toNode(f: Fragment<unknown>): RawNode {
  ...
  if (node === undefined) {
    throw new InvalidFragmentError(
      'value is not a pg-prime Fragment. Fragments can only be produced by the `sql` tag and ' +
...
/**
 * `process` is referenced through `globalThis` rather than imported: `pg-prime` ships with zero
