...
function nullsOf(v: unknown, where: string): 'first' | 'last' | undefined {
  ...
  throw new BuilderError(
    `pg-prime: ${where} takes 'first' or 'last' (got ${JSON.stringify(v)}). The NULLS position is a ` +
      `SQL keyword, emitted as written, so pg-prime will not pass an unrecognised one through.`,
  )
}
