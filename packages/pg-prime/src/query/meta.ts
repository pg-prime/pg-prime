...
export interface TableCodecMeta {
  ...
  /**
   * `keys` minus **GENERATED ALWAYS** (`ddl.identity === 'always'`), in declaration order — the
   * runtime half of the rule the type level already enforces, where `ro: true` removes a column
   * from `InsertPatch`. COPY reads it because it builds its column list from the SCHEMA rather
   * than from the caller's row keys, and had no other way to know (design/13 §5, E's F1).
   * `generatedByDefault()` stays in: `generated by default as identity` is insertable.
   */
