...
export interface TableCodecMeta {
  ...
  /**
   * `keys` minus the columns a write may not name: **GENERATED ALWAYS** (`.generatedAlways()`,
   * `ddl.identity === 'always'`). Declaration order, so it is `keys` with holes and not a re-sort.
   *
   * This is the runtime half of the rule the *type* level already enforces — `ro: true` removes a
   * column from `InsertPatch` / `SetPatch` (`query/types.ts`), so an insert can never name one and
   * a schema author never has to think about it. COPY reads the same set, because it builds its
   * column list from the schema rather than from the caller's row keys and had no other way to
   * know: `copyFrom(posts, rows)` used to send `("id", …)` for a `bigint generated always as
   * identity`, COPY wrote `\N` into it — COPY bypasses the rewriter check that makes an INSERT say
   * `428C9` — and PostgreSQL answered `23502` (design/13 §5, E's F1). `CopyOptions.columns`' own
   * doc comment already said "every column the schema declares as insertable"; this is that set.
   *
   * `generatedByDefault()` is NOT in it — `generated by default as identity` is insertable, which
...
function build(t: TableLike, registry: CodecRegistry): TableCodecMeta {
  ...
  for (const ref of rt.columns) {
    ...
    columns.push(meta)
    keys.push(ref.key)
    if (ref.column.ddl.identity !== 'always') insertableKeys.push(ref.key)
    byKey[ref.key] = meta
