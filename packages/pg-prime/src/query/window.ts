...
function litOffset(n: number): Node {
  ...
  if (!Number.isSafeInteger(n) || n < 0) {
    throw new BuilderError(
      `pg-prime: a window frame offset must be a non-negative safe integer (got ${String(n)})`,
    )
  }
...
export function oneOf<T extends string>(v: unknown, legal: readonly T[], where: string): T {
  ...
  throw new BuilderError(
    `pg-prime: ${where} must be one of ${legal.map((x) => `'${x}'`).join(', ')} (got ` +
      `${JSON.stringify(v)}). PostgreSQL keywords are emitted as written, so pg-prime will not ` +
...
function bound(b: Bound, where: string): FrameBound {
  ...
  throw new BuilderError(
    `pg-prime: ${where} takes ${BOUND_KEYWORDS.map((x) => `'${x}'`).join(', ')}, ` +
...
export function buildWindow(f: WindowFn): WindowDef {
  ...
  if (!(out instanceof Spec)) {
    throw new BuilderError(
      'pg-prime: a window callback must return the spec it was given — `w => w.partitionBy(x)`.',
    )
  }
  return out.def
}
...
export function toWindowDef(v: WindowSpec | WindowLiteral | WindowFn): WindowDef {
  ...
  if (typeof v !== 'object' || v === null) {
    throw new BuilderError(
      'pg-prime: a window definition is `w => w.partitionBy(...)` or ' +
        '`{ partitionBy: [...], orderBy: [...] }`.',
    )
  }
  ...
  if (def.partitionBy === undefined && def.orderBy === undefined && def.frame === undefined) {
    ...
    throw new BuilderError(
      'pg-prime: this window definition is empty. Give it a partitionBy, an orderBy or a frame — ' +
...
export function over<T, P extends string>(e: Expr<T, P>, w: WindowFn | string): Expr<T, P> {
  ...
  if (node.k !== 'agg' && node.k !== 'fn') {
    throw new BuilderError(
      'pg-prime: over() takes an aggregate or a window function — `over(fn.sum(x), w)`, ' +
