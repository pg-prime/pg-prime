 ...
 *
 *   `PG_PRIME_TEST_URL` unset → `_setup.ts` boots this test file its own PGlite behind
 *                            `PGLiteSocketServer` on an ephemeral port. No Docker; identical on
 *                            ubuntu / macos / windows, which is design/08 §4.2's whole point.
 *   `PG_PRIME_TEST_URL` set   → that server, whatever it is, shared by every file and partitioned by
...
const NO_TARGET =
  ...
  'vitest project (packages/pg-prime/vitest.config.ts), not to `unit`. ' +
  'Run `pnpm test:live` (PGlite, no Docker) or `pnpm test:pg` with PG_PRIME_TEST_URL set.'

/** The target *this file* is pointed at. */
export function liveTarget(): LiveTarget {
  if (fileTarget) return fileTarget
  const plan = inject('pgPrimeLivePlan') as LivePlan | undefined
...
export function planProbe(sql: string, major: number = pgMajor()): readonly string[] {
  ...
  const name = `__pgprime_plan_${Math.abs(hashSql(sql))}`
...
export async function makeHarness(max = 4): Promise<Harness> {
  ...
  try {
    ...
    throw new Error(
      `pg-prime live tests could not reach ${liveTarget().url} (${liveTarget().kind}). ` +
...
export function sqlState(e: unknown): string | undefined {
  const raw = e as { code?: string; pgPrime?: { server?: { sqlstate?: string } } } | null
  return raw?.code ?? raw?.pgPrime?.server?.sqlstate
}
 ...
 * live in `test/pg/` and run only against `PG_PRIME_TEST_URL`.
 *
