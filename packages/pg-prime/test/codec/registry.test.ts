...
beforeAll(async () => {
  ...
  await conn.execute({
    text: `create domain pgprime_pos as int4 check (value > 0)`,
    params: [],
    mode: 'simple',
  })
})
...
describe('§4.6 resolveDynamic — user-defined types, resolved by NAME against the catalogue', () => {
  it('resolves an enum and a domain, and derives their array codecs for free', async () => {
    ...
    await registry.resolveDynamic(conn, [
      { name: 'pgprime_mood', kind: 'enum', enumLabels: ['sad', 'ok', 'happy'] },
      { name: 'pgprime_pos', kind: 'domain' },
    ])
    expect(registry.resolved).toBe(true)

    const mood = registry.byName('pgprime_mood')!
    expect(mood.typeClass).toBe('enum')
    expect(mood.oid).toBeGreaterThan(16384) // a user OID — never baked into generated code
    expect(registry.byName('pgprime_mood[]')).toBeDefined()

    // the OID the live server reports for the column is the one we resolved
    const r = await conn.execute({
      text: `select 'happy'::pgprime_mood as m, array['sad','ok']::pgprime_mood[] as ms`,
      params: [],
    })
  ...
  it('a domain delegates to its base type and widens paramOid to `unknown` (705)', async () => {
    const registry = createRegistry()
    await registry.resolveDynamic(conn, [{ name: 'pgprime_pos', kind: 'domain' }])
    const pos = registry.byName('pgprime_pos')!
    ...
    const r = await conn.execute({
      text: 'select $1::pgprime_pos as p',
      params: ['5'],
      paramTypes: [705],
    })
    ...
    await expect(
      conn.execute({ text: 'select $1::pgprime_pos', params: ['-1'], paramTypes: [705] }),
    ).rejects.toMatchObject({ pgPrime: { server: { sqlstate: '23514' } } })
  })

  it('a domain ARRAY column IS reported under its own user OID — so the derived array codec is load-bearing', async () => {
    const registry = createRegistry()
    await registry.resolveDynamic(conn, [{ name: 'pgprime_pos', kind: 'domain' }])
    const arr = registry.byName('pgprime_pos[]')!
    expect(arr.oid).toBeGreaterThan(16384)

    await conn.execute({
      text: 'create temp table dom_arr_t (a pgprime_pos, b pgprime_pos[])',
      params: [],
    })
  ...
  it('a label mismatch is a HARD ERROR at connect, not a runtime surprise', async () => {
    ...
    await expect(
      registry.resolveDynamic(conn, [
        { name: 'pgprime_mood', kind: 'enum', enumLabels: ['sad', 'happy', 'ok'] }, // wrong ORDER
      ]),
  ...
  it('a kind mismatch against pg_type.typtype is a hard error too', async () => {
    const registry = createRegistry()
    await expect(
      registry.resolveDynamic(conn, [{ name: 'pgprime_mood', kind: 'domain' }]),
  ...
  it('a missing type names itself in the error', async () => {
    const registry = createRegistry()
    await expect(
      registry.resolveDynamic(conn, [{ name: 'pgprime_nope', kind: 'enum' }]),
    ).rejects.toThrow(/"pgprime_nope" declared in the schema does not exist/)
  })
})
