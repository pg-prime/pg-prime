 ...
 * (design/09 §2.2).
 *
 *   `PGORM_TEST_URL` set   → that server: probe `server_version_num` here and hand the whole
 *                            target to the workers, so every file shares one database and the
 *                            per-file schema namespaces (R6) keep them apart.
 *   `PGORM_TEST_URL` unset → PGlite, and then the *url* is per test file, not per run — see
...
export async function setup(vitest: Vitest): Promise<void> {
  const url = process.env['PGORM_TEST_URL']

  if (!url) {
    vitest.provide('pgormLivePlan', { kind: 'pglite' })
    console.info(
