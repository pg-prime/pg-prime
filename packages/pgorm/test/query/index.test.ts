/**
 * The package entry point (`src/index.ts`) — what `import { … } from 'pgormjs'` gets.
 *
...
import * as pgorm from '../../src/index.js'
...
describe('the pgormjs barrel', () => {
  it('exports everything design/03 §2 and design/05 name', () => {
    const missing = EXPECTED_VALUES.filter((k) => !(k in pgorm))
    expect(missing).toStrictEqual([])
  })

  it('exports nothing internal', () => {
    const leaked = EXPECTED_ABSENT.filter((k) => k in pgorm)
    expect(leaked).toStrictEqual([])
  })

  it('builds and compiles a query through the barrel alone', () => {
    const users = pgorm.pgTable('users', (t) => ({
      id: t.bigint().primaryKey().generatedAlways(),
      email: t.text(),
      deletedAt: t.timestamptz().nullable(),
    }))
    const schema = pgorm.defineSchema({ users })
    const db = pgorm.compileOnly(schema)
    const built = db
      .from(schema.h.users)
      .select(({ users: u }) => ({ id: u.id, email: u.email }))
      .where(({ users: u }) => pgorm.and(pgorm.isNull(u.deletedAt), pgorm.like(u.email, '%@acme')))
      .orderBy(({ users: u }) => pgorm.desc(u.id))
      .limit(10)
