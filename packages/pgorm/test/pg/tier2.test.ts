 ...
 * `PGORM_TEST_URL` (`pnpm test:pg`) and every case must run.
 */
...
/** Arbitrary, but stable and ours: `pgorm` in ASCII, shifted into the advisory-lock key space. */
...
describe('tier 2 — two connections are two backends', () => {
  ...
  requiresConcurrency()('session state does not leak between them', async () => {
    await a.execute({ text: `select set_config('pgorm.probe', 'a', false)`, params: [] })
    expect(await one(b, `select current_setting('pgorm.probe', true)`)).toBeNull()
  })
  ...
  requiresConcurrency()('a row locked by one is skipped, not returned, by `skip locked`', async () => {
    await a.execute({ text: 'drop table if exists pgorm_tier2_q', params: [], mode: 'simple' })
    await a.execute({
      text: `create table pgorm_tier2_q (id int primary key)`,
      params: [],
      mode: 'simple',
    })
    await a.execute({ text: `insert into pgorm_tier2_q values (1), (2)`, params: [] })
    await a.execute({ text: 'begin', params: [] })
    try {
      expect(await one(a, 'select id from pgorm_tier2_q where id = 1 for update')).toBe('1')
      // The queue-workload guarantee: B must see 2 and never block on 1.
      const rows = (
        await b.execute({
          text: 'select id from pgorm_tier2_q order by id for update skip locked',
          params: [],
        })
      ).rows.map((r) => r[0])
      ...
      await a.execute({ text: 'drop table pgorm_tier2_q', params: [], mode: 'simple' })
    }
  })
})
