...
export class InvalidIdentifierError extends PgOrmError {
  ...
  constructor(reason: IdentRejectReason, index: number, part: unknown, detail: string) {
    super(
      'INVALID_IDENTIFIER',
      `pgorm: identifier part ${index} rejected (${reason}): ${detail}`,
    )
    this.reason = reason
    this.index = index
    this.part = part
 ...
 */
export class NoCodecError extends PgOrmError {
  ...
  constructor(table: string, column: string, pgType: string, hint: string) {
    super(
      'NO_CODEC',
      `pgorm: column "${table}"."${column}" is declared as PostgreSQL type '${pgType}', ` +
...
export class NullOperandError extends PgOrmError {
  readonly operator: string
  constructor(operator: string) {
    super(
      'NULL_OPERAND',
      `pgorm: ${operator}(a, null) would compile to \`a ${OPERATOR_SYMBOL[operator] ?? '='} NULL\`, ` +
 ...
 */
export class DecodePlanError extends PgOrmError {
  constructor(message: string) {
 ...
 * errors obey).
 */
export class BuilderError extends PgOrmError {
  constructor(message: string) {
 ...
 * the relation.
 */
export class SchemaError extends PgOrmError {
  constructor(message: string) {
