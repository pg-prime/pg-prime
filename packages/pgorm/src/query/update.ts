...
export class UpdateBuilder {
    ...
    for (const item of added) {
      if (already.has(item.column.name)) {
        throw new BuilderError(
          `pgorm: "${item.column.name}" is already assigned by an earlier .set() — PostgreSQL ` +
  ...
  fromValues(
    ...
    if (rows.length === 0) {
      throw new BuilderError('pgorm: fromValues([]) would match no rows; nothing to update.')
    }
    const alias = checkAlias(opts.alias ?? 'v')
    if (this.s.scope[alias] !== undefined) {
      throw new BuilderError(
        `pgorm: alias "${alias}" is already in scope — the fromValues() relation would shadow ` +
    ...
    if (strategy === 'unnest') {
      ...
      if (arrayKey !== undefined) {
        ...
        throw new BuilderError(
          `pgorm: fromValues({ strategy: 'unnest' }) cannot carry the array column ` +
  ...
  toAst(): UpdateNode {
    if (this.s.set.length === 0) {
      throw new BuilderError('pgorm: update(...) needs a .set({...}).')
    }
    if (this.s.where === undefined && !this.s.allRows) {
      throw new BuilderError(
        `pgorm: update("${this.s.target.table.name}") has no .where(), so it would rewrite every ` +
...
function literalValues(
  ...
  return valuesFrom({
    ...
    rows: rows.map((row, r) =>
      keys.map((k) => {
        if (!(k in row)) throw new BuilderError(`pgorm: row ${r} of fromValues() does not set "${k}".`)
...
export function makeUpdate(
  ...
  if (source.kind !== 'table') {
    throw new BuilderError('pgorm: update() takes a table handle, not a CTE or derived table.')
  }
