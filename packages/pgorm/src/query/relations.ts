...
function tableOf(ctx: BuilderCtx, key: string, rel: ResolvedRelation): AnyTable {
  ...
  if (t === undefined) {
    throw new BuilderError(
      `pgorm: relation "${rel.parent}.${rel.name}" points at table "${key}", which this ` +
...
function refAt(scope: RefScope, key: string, where: string): RefNode {
  const r = scope[key]
  if (r === undefined) throw new BuilderError(`pgorm: ${where} has no column "${key}".`)
  return r
}
...
function runSub(corr: Correlated, f: ((q: never) => unknown) | undefined): SubState {
  ...
  if (!(out instanceof RelSub)) {
    throw new BuilderError(
      'pgorm: a relation sub-query callback must return the sub-query it was given — ' +
