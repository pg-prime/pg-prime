...
function runnerOf(ctx: BuilderCtx): NonNullable<BuilderCtx['runner']> {
  if (ctx.runner === undefined) {
    throw new BuilderError(
      'pgorm: this query has no executor, so it can be compiled but not executed. Build it from ' +
        '`pgOrm({ driver, schema })` rather than from `compileOnly(schema)`.',
    )
  }
  return ctx.runner
}
...
export class SelectBuilder {
  ...
  window(name: string, f: Lambda<WindowSpec | WindowLiteral | WindowFn>): SelectBuilder {
    if (this.s.windows.some((w) => w.name === name)) {
      throw new BuilderError(
        `pgorm: a window named "${name}" is already declared on this query. Rename one.`,
      )
    }
  ...
  #join(
    ...
    if (onFn === undefined) {
      throw new BuilderError(`pgorm: ${type} join on "${alias}" needs an ON predicate.`)
    }
  ...
  asScalar(): unknown {
    ...
    if (items === undefined || items.length !== 1) {
      throw new BuilderError(
        `pgorm: asScalar() needs a projection with exactly one column (got ${items?.length ?? 0}).`,
      )
    }
    ...
    if (only.group !== undefined || only.nested !== undefined) {
      throw new BuilderError(
        `pgorm: asScalar() needs one *column*; "${only.key}" is a ` +
...
function needsSelect(): BuilderError {
  return new BuilderError(
    'pgorm: .select() is required before this query can be used — there is no implicit ' +
...
function aliasTaken(alias: string): BuilderError {
  return new BuilderError(
    `pgorm: alias "${alias}" is already in scope. Give the second one a name — ` +
      `.innerJoin(t, 'other', …).`,
  )
}
...
export function checkAlias(alias: string): string {
  if (RESERVED_ALIAS.test(alias)) {
    throw new BuilderError(
      `pgorm: "${alias}" is reserved — the compiler names its own hoisted subqueries _r1, _r2, ` +
...
function unknownAlias(alias: string, scope: Scope): BuilderError {
  return new BuilderError(
    `pgorm: no alias "${alias}" in scope (have: ${Object.keys(scope).join(', ') || 'none'}).`,
  )
}
