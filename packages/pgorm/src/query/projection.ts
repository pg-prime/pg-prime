...
export function assertSafeKey(key: string, where: string): string {
  if (key === '__proto__') {
    throw new BuilderError(
      `pgorm: "__proto__" cannot be used as a ${where} key — it sets an object's prototype ` +
...
export function toExprNode(v: unknown, where: string): Node {
  ...
  if (isNested(v)) {
    throw new BuilderError(
      `pgorm: ${where} is a relation projection, which is only valid directly in a projection ` +
        `record — not inside an expression. Give it its own key.`,
    )
  }
  throw new BuilderError(
    `pgorm: ${where} is not an expression (got ${describe(v)}). A projection takes a column ` +
      `reference, an operator result, \`val(value, codec)\`, or \`sql\`…\`.as(codec)\` — a bare ` +
      `value has no PostgreSQL type and pgorm will not guess one.`,
  )
}
