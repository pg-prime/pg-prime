...
function runnerOf(ctx: BuilderCtx): NonNullable<BuilderCtx['runner']> {
  if (ctx.runner === undefined) {
    throw new BuilderError('pgorm: this statement has no executor; it can be compiled, not executed.')
  }
  return ctx.runner
}
...
function valueExpr(v: unknown, codec: AnyCodec, where: string): Node {
  ...
  if (isNested(v)) {
    throw new BuilderError(
      `pgorm: ${where} is a relation projection, which is only valid in a projection or a ` +
        `RETURNING list — not as a value to write.`,
    )
  }
  if (isGroup(v)) {
    throw new BuilderError(
      `pgorm: ${where} is a nest({...}) group, which is a result-shaping marker and not a ` +
...
export class InsertBuilder {
  ...
  valuesMany(rows: readonly Record<string, unknown>[], opts?: BulkOpts): InsertBuilder {
    if (rows.length === 0) {
      throw new BuilderError('pgorm: valuesMany([]) has no columns to insert; nothing to do.')
    }
  ...
  onConflict(f: (c: ConflictSpec) => ConflictSpec): InsertBuilder {
    ...
    if (!(out instanceof ConflictSpec)) {
      throw new BuilderError('pgorm: the onConflict callback must return the spec it was given.')
    }
  ...
  toAst(): InsertNode {
    if (this.#ast === undefined) {
      ...
      if (statements.length > 1) {
        throw new BuilderError(
          `pgorm: this bulk insert is ${statements.length} statements (chunked at ` +
  ...
  #strategy(rows: readonly Record<string, unknown>[]): 'values' | 'unnest' {
    ...
    if (arrayColumn !== undefined) {
      if (declared === 'unnest') {
        throw new BuilderError(
          `pgorm: the 'unnest' bulk strategy cannot insert the array column ` +
  ...
  #statements(): readonly InsertNode[] {
    ...
    if (rows === undefined) {
      throw new BuilderError(
        'pgorm: insertInto(...) needs .values(...), .valuesMany(...), .fromSelect(...) or ' +
          '.defaultValues().',
      )
    }
  ...
  #columnsFor(rows: readonly Record<string, unknown>[]): readonly ColumnMeta[] {
    ...
    for (let r = 1; r < rows.length; r++) {
      ...
      if (rowKeys.length !== keys.length || rowKeys.some((k) => !Object.hasOwn(first, k))) {
        throw new BuilderError(
          `pgorm: every row of a bulk insert must set the same columns. Row ${r} sets ` +
...
function unknownColumns(where: string, keys: readonly string[], table: string): BuilderError {
  return new BuilderError(
    `pgorm: ${where} names column(s) [${keys.join(', ')}] that "${table}" does not have.`,
  )
}
...
function checkBulkOpts(opts: BulkOpts): void {
  ...
  if (strategy !== undefined && strategy !== 'auto' && strategy !== 'values' && strategy !== 'unnest') {
    throw new BuilderError(
      `pgorm: unknown bulk strategy ${JSON.stringify(strategy)} — it is 'auto' (the default), ` +
        `'values' or 'unnest'.`,
    )
  }
  ...
  if (size !== undefined && (!Number.isInteger(size) || size < 1)) {
    ...
    throw new BuilderError(
      `pgorm: chunkSize must be a positive integer (got ${String(size)}).`,
    )
  }
}

function missingKey(key: string, row: number): BuilderError {
  return new BuilderError(`pgorm: row ${row} of this insert does not set "${key}".`)
}
...
function mergeCtes(outer: readonly CteNode[], inner: readonly CteNode[]): readonly CteNode[] {
  ...
  for (const c of inner) {
    ...
    if (existing !== undefined) {
      throw new BuilderError(
        `pgorm: two different CTEs are named "${c.name}" in one statement. Rename one — a ` +
...
export class ConflictSpec {
  ...
  columns(f: Lambda<unknown>): ConflictSpec {
    ...
    const columns = refs.map((r) => {
      if (r.k !== 'col') {
        throw new BuilderError('pgorm: onConflict().columns() takes column references.')
      }
      const key = keyOfColumn(this.#meta, r.name)
      if (this.#target[key] !== r) {
        throw new BuilderError(
          `pgorm: onConflict().columns() takes columns of "${this.#meta.table.name}" itself; ` +
  ...
  where(f: Lambda<unknown>): ConflictSpec {
    ...
    if (target === undefined || target.k === 'constraint') {
      throw new BuilderError(
        'pgorm: onConflict().where() is the partial-index predicate, so it needs .columns(...) ' +
  ...
  whereUpdate(f: (t: RefScope, excluded: RefScope) => unknown): ConflictSpec {
    ...
    if (action.k !== 'update') {
      throw new BuilderError('pgorm: onConflict().whereUpdate() needs .doUpdate(...) first.')
    }
...
function keyOfColumn(meta: TableCodecMeta, name: string): string {
  ...
  if (i < 0) throw new BuilderError(`pgorm: no column "${name}" on "${meta.table.name}".`)
...
export function setItemsOf(
  ...
  if (unknownKeys.length > 0) {
    throw new BuilderError(
      `pgorm: ${where} names column(s) [${unknownKeys.join(', ')}] that "${meta.table.name}" ` +
        `does not have.`,
    )
  }
  return Object.freeze(out)
...
export function makeInsert(
  ...
  if (source.kind !== 'table') {
    throw new BuilderError('pgorm: insertInto() takes a table handle, not a CTE or derived table.')
  }
