...
export class DeleteBuilder {
  ...
  using(h: object, alias?: string): DeleteBuilder {
    ...
    if (this.s.scope[name] !== undefined) {
      ...
      throw new BuilderError(
        `pgorm: alias "${name}" is already in scope. Give the USING item a name — ` +
          `.using(t, 'other').`,
      )
    }
  ...
  toAst(): DeleteNode {
    if (this.s.where === undefined && !this.s.allRows) {
      throw new BuilderError(
        `pgorm: deleteFrom("${this.s.from.table.name}") has no .where(), so it would delete ` +
...
export function makeDelete(
  ...
  if (source.kind !== 'table') {
    throw new BuilderError('pgorm: deleteFrom() takes a table handle, not a CTE or derived table.')
  }
  const name = checkAlias(alias ?? source.name)
