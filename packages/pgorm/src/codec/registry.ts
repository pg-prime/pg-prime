  ...
  async resolveDynamic(
    ...
    for (const req of requests) {
      ...
      if (candidates.length > 1) {
        throw new Error(
          `pgorm: type "${req.name}" is ambiguous on this search_path — it exists in [${candidates
            .map((c) => c.nspname)
    ...
    for (const req of requests) {
      ...
      if (req.kind === 'enum') {
        ...
        // `int4`'s 1007 made `arrayCodecOf(domainCodec)` claim that a `pgorm_pos[]` value is an
