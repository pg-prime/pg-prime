...
class ColumnBuilder implements ColumnRuntime, Bodies {
  ...
  #reject(what: string, why: string): never {
    throw new SchemaError(`pgorm: ${what} — ${why}. Drop one of the two.`)
  }
...
export function checkName(value: unknown, what: string): void {
  try {
    ...
    if (e instanceof InvalidIdentifierError) {
      throw new SchemaError(`pgorm: ${what} is not a usable PostgreSQL identifier: ${e.reason} (${e.message}).`)
    }
    throw e
  }
}
  ...
  for (const label of values) {
    ...
    if (seen.has(label)) {
      throw new SchemaError(
        `pgorm: pgEnum("${name}") declares the label "${label}" twice. PostgreSQL rejects a ` +
