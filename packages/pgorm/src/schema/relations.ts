  ...
  for (const parent of Object.keys(rels)) {
    ...
    if (table === undefined) {
      throw new SchemaError(
        `pgorm: defineRelations declares relations on "${parent}", which is not a table in this ` +
    ...
    for (const name of Object.keys(declared)) {
      ...
      if (columns.has(name)) {
        throw new SchemaError(
          `pgorm: relation "${parent}.${name}" has the same name as a column of "${parent}". ` +
...
function one(
  ...
  if (target === undefined) {
    throw new SchemaError(
      `pgorm: ${where} points at "${node.to}", which is not a table in this schema ` +
  ...
  if (cfg === undefined || cfg.from === undefined || cfg.to === undefined) {
    throw new SchemaError(
      `pgorm: ${where} needs explicit \`from\` and \`to\` column references — ` +
  ...
  if (cfg.through !== undefined) {
    ...
    if (t.table === undefined || t.from === undefined || t.to === undefined) {
      throw new SchemaError(
        `pgorm: ${where} declares \`through\` but not all of { table, from, to }. The junction ` +
...
function compat(
  ...
  for (let i = 0; i < a.length; i++) {
    ...
    throw new SchemaError(
      `pgorm: ${where} correlates \`${an}\` ${ta.$.name}.${ak} (${at}) with \`${bn}\` ` +
...
function arity(a: readonly string[], b: readonly string[], where: string, an: string, bn: string): void {
  if (a.length !== b.length) {
    throw new SchemaError(
      `pgorm: ${where} pairs ${a.length} column(s) in \`${an}\` with ${b.length} in \`${bn}\`. ` +
...
function keysOf(v: unknown, table: AnyTable, where: string): readonly string[] {
  ...
  if (list.length === 0) throw new SchemaError(`pgorm: ${where} is an empty list.`)
  const keys: string[] = []
  for (const item of list) {
    const ref = item as AnyRef | undefined
    if (ref === undefined || ref === null || typeof ref !== 'object' || ref.$ === undefined) {
      throw new SchemaError(
        `pgorm: ${where} is not a column reference. Write \`myTable[REFS].columnName\`.`,
      )
    }
    if (ref.$.table !== table.$.name) {
      throw new SchemaError(
        `pgorm: ${where} is a column of "${ref.$.table}", but it must be a column of ` +
          `"${table.$.name}".`,
      )
    }
    keys.push(ref.$.key)
...
function fnOr(v: unknown, where: string): ((t: never) => unknown) | undefined {
  ...
  if (typeof v !== 'function') {
    throw new SchemaError(`pgorm: ${where} must be a callback taking the relation's own scope.`)
  }
