...
if (!process.env["PG_PRIME_PG_DUMP"]) {
  ...
  if ("unavailable" in direct) {
    ...
    process.env["PG_PRIME_PG_DUMP_URI"] = `postgresql://${encodeURIComponent(ADMIN.user)}@127.0.0.1:5432/{db}`;
  }
}
