...
describe("migration ids are validated before they become filenames", () => {
  ...
  it("refuses to write outside outDir even when handed a doctored plan", async () => {
    const dir = await mkdtemp(join(tmpdir(), "pg-prime-gate-"));
...
describe("generate() distinguishes a refusal from an I/O failure", () => {
  const CUR = "pgprime_gate_cur";
  const DES = "pgprime_gate_des";
  const T = 180_000;
  ...
  it(
    ...
    async () => {
      const dir = await mkdtemp(join(tmpdir(), "pg-prime-io-"));
  ...
  it(
    ...
    async () => {
      const outDir = join(await mkdtemp(join(tmpdir(), "pg-prime-io-")), "plans");
