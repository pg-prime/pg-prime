...
beforeAll(async () => {
  ...
  expect(
    report.error === undefined ? "" : `${report.error.message} — ${report.error.sql}`,
  ).toBe("");
...
describe("emit → load → extract → re-emit → load (R1)", () => {
  ...
  it("converges in ONE step: the IR of B is the IR of A, fingerprint included", async () => {
  ...
  it("every declared object is in the IR, in the right schema", () => {
    const has = (kind: "table" | "type", schema: string, name: string): boolean =>
      irA.ir.has({ kind, schema, name });
  ...
  it("carries identity, defaults and comments", async () => {
...
describe("the same corpus through the tier-3 schema map", () => {
  it("produces the identical desired IR", async () => {
