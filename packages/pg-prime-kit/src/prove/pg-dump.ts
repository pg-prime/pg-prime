...
/**
 * `PG_PRIME_PG_DUMP` accepts a JSON array for exact argv, or a whitespace-separated string.
 *
...
/**
 * Locate a usable `pg_dump`. Resolution order: explicit launcher, then `PG_PRIME_PG_DUMP`,
...
export async function resolvePgDump(
  ...
  const envRaw = process.env["PG_PRIME_PG_DUMP"];
  const envUri = process.env["PG_PRIME_PG_DUMP_URI"];
  const envArgv = envRaw === undefined ? null : parseLauncherEnv(envRaw);
  if (explicit === undefined && envRaw !== undefined && envArgv === null) {
    return { unavailable: `PG_PRIME_PG_DUMP is not a non-empty argv (JSON string array or whitespace-separated)` };
  }
