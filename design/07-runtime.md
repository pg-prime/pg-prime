...
1. **`DECLARE … CURSOR` with bind parameters over the extended protocol** — the one factual gap I could not close here (no PostgreSQL available in this environment; `psql` is not installed). It decides whether `.stream()` is zero-dep or needs `pg-cursor` as an optional peer. The API is identical either way, so it is not blocking, but it must be settled against a live PG 14–18 matrix in week 1. **This is my trickiest unresolved question.**
